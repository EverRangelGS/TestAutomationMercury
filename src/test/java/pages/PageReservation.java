...
public class PageReservation {
    ...
    public void assertPage(){
        Assert.assertTrue(driver.findElement(titleText).getText().contains("Flight Finder to search"),"No contiene el texto...Flight Finder to search");
    }

    public void selectPassengers (int cant) {
        WebDriverWait wait = new WebDriverWait(driver,10);
        WebElement cantidadPasajeros = wait.until(ExpectedConditions.presenceOfElementLocated(passengersDrop)); //Esperar hasta que el elemento este presente

        Select selectPasajeros = new Select(cantidadPasajeros); //utilizo un combo con el identificador definido
